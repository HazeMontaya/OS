...
export function createAgent({ tools, providers, config }) {
  async function runToolLoop(prompt, preferredProvider, preferredModel) {
    ...
    for (let i = 0; i < config.autonomy.maxToolIterations; i++) {
      ...
      catch { return { provider: generated.provider, model: generated.model, answer: generated.text, iterations: i + 1, toolResults: [] }; }
      if (parsed.action === 'answer') return { provider: generated.provider, model: generated.model, answer: String(parsed.answer || ''), iterations: i + 1 };
      if (parsed.action !== 'tool' || !tools.has(parsed.tool)) return { provider: generated.provider, model: generated.model, answer: generated.text, iterations: i + 1 };
  ...
  return {
    run: async ({ prompt, provider, model, remember = false }) => {
      ...
      const result = await runToolLoop(String(prompt), provider, model);
      if (remember && result.answer) addMemory({ text: result.answer, type: 'episodic', source: 'agent', tags: ['agent-output'], metadata: { provider: result.provider, model: result.model } });
      return result;
    }
  };
}
