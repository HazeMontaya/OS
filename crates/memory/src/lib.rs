...
#[cfg(test)]mod tests{
 use super::*;
 #[test]fn journal_is_idempotent(){let path=std::env::temp_dir().join(format!("haze-memory-idem-{}.log",std::process::id()));let mut m=Memory::default();m.remember("a","note","x");m.append_journal(&path).unwrap();m.append_journal(&path).unwrap();let loaded=Memory::load_journal(&path).unwrap();assert_eq!(loaded.all().len(),1);let _=std::fs::remove_file(path);}
