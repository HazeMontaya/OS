...
#[cfg(test)]mod tests{use super::*;#[test]fn journal_is_idempotent(){let path=std::env::temp_dir().join(format!("haze-events-idem-{}.log",std::process::id()));let mut l=EventLog::default();l.push(Event::RevenueRecorded{cents:42,memo:"x".into()});l.append_journal(&path).unwrap();l.append_journal(&path).unwrap();let loaded=EventLog::load_journal(&path).unwrap();assert_eq!(loaded.len(),1);let _=std::fs::remove_file(path);}#[test]fn journal_roundtrip(){let path=std::env::temp_dir().join(format!("haze-events-{}.log",std::process::id()));let mut l=EventLog::default();l.push(Event::RevenueRecorded{cents:42,memo:"a\tb".into()});l.append_journal(&path).unwrap();let loaded=EventLog::load_journal(&path).unwrap();assert_eq!(loaded.all(),l.all());let _=std::fs::remove_file(path);}}