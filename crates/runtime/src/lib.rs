...
use os_planner::{GoalPlanner, Planner,PlannerInput,RulePlanner};
...
impl Runtime{
 ...
 pub fn create_task_goal(&mut self,id:String,title:String,description:String,action:os_orchestration::GoalAction,owner_agent:String,depends_on:std::collections::BTreeSet<String>)->Result<(),String>{
  self.goals.add_goal(Goal{id,title,description,kind:os_orchestration::GoalKind::Task,action:Some(action),owner_agent:Some(owner_agent),status:GoalStatus::Proposed,parent_id:Some("mission:james".into()),depends_on})
 }
 ...
 pub fn materialize_ready_goals(&mut self)->usize{
  let goals=self.goal_planner.compile(&self.goals).into_iter().filter_map(|step|{ let g=self.goals.goals.get(&step.goal_id)?; if g.status!=GoalStatus::Proposed { return None; } Some((g.id.clone(),g.action,g.owner_agent.clone())) }).collect::<Vec<_>>();
  let mut count=0;
...
#[test]fn goal_materializes_and_completes(){let mut r=Runtime::new(100_000);let result=r.run_cycle();assert_eq!(result.kind,"task");assert_eq!(r.goals.goals.get("goal:boot-health").unwrap().status,GoalStatus::Completed);assert!(r.events.all().iter().any(|e|matches!(e,Event::GoalMaterialized{goal_id,..} if goal_id=="goal:boot-health")));assert!(r.events.all().iter().any(|e|matches!(e,Event::GoalCompleted{goal_id,success:true} if goal_id=="goal:boot-health")));}
 #[test]fn goal_state_survives_restart(){let path=std::env::temp_dir().join(format!("haze-goal-state-{}.tsv",std::process::id()));let mut r=Runtime::new(100_000);r.run_cycle();assert_eq!(r.goals.goals.get("goal:boot-health").unwrap().status,GoalStatus::Completed);r.save_state(&path).unwrap();let mut n=Runtime::new(1);n.load_state(&path).unwrap();assert_eq!(n.goals.goals.get("goal:boot-health").unwrap().status,GoalStatus::Completed);let _=std::fs::remove_file(&path);let _=std::fs::remove_file(path.with_extension("system.json"));let _=std::fs::remove_file(path.with_extension("orchestration.json"));}
